...
## Fix 5: ~~Force fresh Clang/LLVM build~~ (REVERTED)

**This was a mistake!** The Clang cache (v1) was working fine. Bumping to v2 unnecessarily forced a ~2 hour rebuild.

**Reverted**: Kept Clang at v1, but added `save-always: true` to ensure it's saved even if later steps fail.
...
## Fix 8: Clang cache not saved when jank build fails

**Problem**: Clang is rebuilt every CI run (~2 hours) even though it hasn't changed.

**Root cause**: `actions/cache@v4` only saves the cache if the **entire job succeeds**. Since the jank build step fails with the TLS error, the job fails, and the Clang cache is never saved.

Flow:
1. Cache Clang step - cache miss, builds Clang (2 hours)
2. Cache jank step - cache miss
3. Build jank step - FAILS with TLS error
4. Job fails → Clang cache NOT saved
5. Next run: Clang cache miss again → rebuilds!

**Solution**: Add `save-always: true` to the Clang cache:
```yaml
- name: Cache Clang/LLVM
  ...
  with:
    ...
    key: clang-linux-x64-${{ env.JANK_REF }}-v2
    save-always: true  # Save even if later steps fail
```

**Lesson**: For expensive builds that complete before the failing step, use `save-always: true` to preserve the cache even on job failure.

## Current Status: Linux CI Disabled

The Linux CI is temporarily disabled due to an unresolved jank issue with emulated TLS on Linux.

**Error**: `JIT session error: Symbols not found: [ __emutls_v._ZN4jank7runtime17current_allocatorE ]`

**Attempted fixes**:
1. Added `[[gnu::visibility("default")]]` to `current_allocator` - didn't work
2. Added `[[gnu::used]]` attribute - didn't work

**Root cause**: On Linux, thread_local variables use emulated TLS (with `__emutls_v.*` symbols). These symbols aren't being properly exported to the ORC JIT's symbol table, even with visibility and used attributes. This needs to be fixed in jank's JIT processor to explicitly register emulated TLS symbols.

**Workaround**: Linux jobs disabled with `if: false` until jank fixes the issue upstream.
